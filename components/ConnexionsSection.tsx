...
import type { QontoAccountSafe, ShineAccountSafe, BankAccountInfo } from '@/app/(dashboard)/integrations/actions'
...
type ConnexionsTab = 'qonto' | 'shine' | 'rapprochement'
...
export function ConnexionsSection() {
  const [tab, setTab] = useState<ConnexionsTab>('qonto')
  return (
    <div>
      <div className="settings-section-header">
        <h2 className="settings-section-title">Connexions API</h2>
        <p className="settings-section-desc">Synchronisez vos transactions bancaires automatiquement</p>
      </div>
      <div className="dash-tabs" style={{ marginBottom: 20 }}>
        <button className={`dash-tab ${tab === 'qonto' ? 'dash-tab-active' : ''}`} onClick={() => setTab('qonto')}>Qonto</button>
        <button className={`dash-tab ${tab === 'shine' ? 'dash-tab-active' : ''}`} onClick={() => setTab('shine')}>Shine</button>
        <button className={`dash-tab ${tab === 'rapprochement' ? 'dash-tab-active' : ''}`} onClick={() => setTab('rapprochement')}>Rapprochement</button>
      </div>
